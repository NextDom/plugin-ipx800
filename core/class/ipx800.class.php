...
class ipx800 extends eqLogic
{
    ...
    /**
     * 
     * @param type $url_serveur
     * @param type $pathjeedom
     * @param type $ipjeedom
     * @param type $portjeeom
     * @param type $seuil_base
     * @param type $seuil_haut
     * @throws \Exception
     * @throws \Exception
     */
     ...
     * @throws \Exception
     */
    public function configPushGet($urlServeur)
    {
        $gceid       = substr($this->getLogicalId(), strpos($this->getLogicalId(), "_") + 2);
        $urlServeur .= 'protect/assignio/analog' . ($gceid + 1) . '.htm';
        log::add('ipx800', 'debug', "get " . preg_replace("/:[^:]*@/", ":XXXX@", $urlServeur));
        $result      = @file_get_contents($urlServeur);
        if ($result === false) {
            throw new \Exception(__('L\'ipx ne repond pas.', __FILE__));
        }
...
class ipx800Cmd extends cmd
{
    ...
    public function execute($options = null)
    {
        ...
        switch ($eqLogic->getConfiguration('type', '')) {
            ...
            case "relai":
                ...
                } elseif ($this->getLogicalId() == 'btn_off') {
     ...
     * @return type
     */
    public function event($value, $datetime = null, $loop = 1)
    {
        if ($this->getLogicalId() == 'nbimpulsionminute') {
            try {
                $calcul = $this->getConfiguration('calcul');
                $calcul = preg_replace("/#brut#/", $value, $calcul);
                $result = jeedom::evaluateExpression($calcul);
                parent::event($result, $datetime, $loop);
            } catch (Exception $e) {
                $EqLogic = $this->getEqLogic();
                log::add('ipx800', 'error', $EqLogic->getName() . " error in " . $this->getConfiguration('calcul') . " : " . $e->getMessage());
                return scenarioExpression::setTags(str_replace('"', '', cmd::cmdToValue($this->getConfiguration('calcul'))));
            ...
            parent::event($value, $datetime, $loop);
        }
    }
